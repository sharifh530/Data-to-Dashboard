...
- CSV accepts UTF-8/BOM, treats the first row as headers and infers comma, semicolon, tab or pipe. Ambiguous detection falls back to comma. A user can explicitly choose one of those four delimiters and re-inspect the same raw bytes; at most three delimiter revisions are allowed per upload. Values remain strings, preserving leading zeros. Empty/duplicate headers produce a warning; inconsistent widths and malformed quoting are rejected. Encoding and header-row overrides are not implemented.
...
`POST /api/v1/datasets/{id}/inspection` requires ownership, session, exact Origin and CSRF. It is idempotent for the current delimiter and admits one queued/running inspection per owner. `GET` is owner-only. The cancel endpoint cancels queued jobs; running jobs return 409 and finish under their deadline. A failed/rejected/cancelled result can be reinspected with a different delimiter within the three-revision budget; retrying the same setting requires a new upload.

The optional inspection body is `{ "delimiter": ";" }` (or comma, tab, pipe, or `null` for automatic detection). Changing it after completion queues a fresh inspection, clears the old report and current table choice, and preserves original bytes. The fixed parser receives only a validated delimiter code through the Windows-to-WSL bridge; it never receives arbitrary parsing instructions. `POST /api/v1/datasets/{id}/inspection/selection` with `{ "table": "..." }` accepts only a name in the ready report. It saves the selected table and creates a `dataset_versions` row with the table, detected/forced delimiter, raw SHA-256 and a schema fingerprint. The inspection response returns `dataset_version_id`. Choosing the same table again is idempotent. This is configuration provenance for future analysis; it does not start an analysis job.
...
Remaining B05 work: encoding/header overrides and broader dataset acceptance decisions. The selected table and delimiter are now durable analysis input configuration. B07 profiling/artifact manifests follow this work. Full generated execution, model evaluation, production deployment and comprehensive hostile-runtime testing remain separate milestones.
