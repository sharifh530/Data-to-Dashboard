...
September 18 addition: **Inspect dataset** queues fixed CSV/SQLite parsing in gVisor. The owner-only preview shows table choice, row/column/empty counts and five rows. The table selector changes the preview only; persistent analysis configuration is still future work. See [setup and limits](INSPECTION.md). The browser acceptance suite can exercise real inspection when `DTD_TEST_INSPECTION_IMAGE` is configured.
