...
test('real local sign-in, upload, history, cancellation, refresh and logout', async ({ page }) => {
  ...
  if (process.env.DTD_TEST_INSPECTION_IMAGE) {
    ...
    await page.getByRole('button', { name: 'Use this table' }).click();
    await page.getByRole('button', { name: 'Profile selected table' }).click();
    await expect(page.getByText('Profile: ready', { exact: true })).toBeVisible({ timeout: 60000 });
    await expect(page.getByText('245 rows profiled · 11 columns')).toBeVisible();
  }
